...
export default function CaseStudies() {
  ...
  const BlogCard = ({
    ...
    >
      ...
      >
        <div className="bg-[#EDF5D4] p-6 rounded-full">
          <ArrowUpRight className="w-6 h-6 transition-transform duration-300 group-hover:-translate-y-0.5 group-hover:translate-x-0.5" />
        </div>
      </div>
      <CardContent className="bg-white rounded-2xl h-full flex flex-col p-0 justify-evenly">
        <div
        ...
        >
          <Image
            src={data.image}
            alt={data.title}
            width={isMobile ? 200 : data.width}
            height={isMobile ? 460 : data.height}
