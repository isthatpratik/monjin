...
const blogData = [
  ...
  },
  {
    ...
    description: "To resolve talent acquisition challenges in the Retail Industry",
    width: 240,
    height: 144,
    span: 3
  },
...
export default function BlogSection() {
  ...
  const BlogCard = ({ data, isMobile }: { data: typeof blogData[0], isMobile: boolean }) => (
    <Card className={`bg-[#F7FAF9] rounded-2xl p-2 shadow-lg relative group cursor-pointer transition-all duration-300 transform hover:scale-102 hover:shadow-xl hover:translate-x-1 hover:translate-y-1 ${isMobile ? 'h-[400px] w-full' : 'h-full'}`}>
      <div className={`absolute -top-4 -right-4 rounded-full p-2 bg-white ${isMobile ? 'hidden' : 'block'}`}>
      ...
      <CardContent className="bg-white rounded-2xl h-full flex flex-col p-0">
        <div className={`flex-1 align-middle flex justify-center items-center p-2 ${isMobile ? 'h-1/2' : ''}`}>
          <Image
            src={data.image}
            alt={data.title}
        ...
        <div className={`flex flex-col justify-end p-4 ${isMobile ? 'h-1/2' : 'my-2'}`}>
